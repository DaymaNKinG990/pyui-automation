...
class TestMouse:
    ...
    def test_move_valid_coordinates(self, mock_backend):
        mouse = Mouse(mock_backend)
        assert mouse.move(100, 200) is True
        mock_backend.move_mouse.assert_called_once_with(100, 200)

    def test_move_invalid_coordinates(self, mock_backend):
        mouse = Mouse(mock_backend)
        with pytest.raises(ValueError, match="Coordinates must be numbers"):
            mouse.move("invalid", 200)

    def test_drag_success(self, mock_backend):
        mouse = Mouse(mock_backend)
    ...
    def test_drag_move_fails(self, mock_backend):
    ...
    def test_drag_invalid_coordinates(self, mock_backend):
        mouse = Mouse(mock_backend)
        with pytest.raises(ValueError, match="Coordinates must be numbers"):
...
# Keyboard Tests
class TestKeyboard:
    def test_type_text_success(self, mock_backend):
